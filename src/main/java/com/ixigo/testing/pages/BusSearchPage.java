...
public class BusSearchPage {
    ...
    @FindBy(xpath = "//span[.='Seats']")        
    private WebElement seats;
    @FindBy(xpath = "//span[.='Ratings']")       
    private WebElement ratings;
    @FindBy(xpath = "//span[.='Arrival Time']") 
    private WebElement arrivaltime;
    @FindBy(xpath = "//span[.='Departure Time']") 
    private WebElement depaturetime;
    @FindBy(xpath = "(//img[@width='auto' and @height='100'])[2]")
    private WebElement assuredfilter;
    @FindBy(xpath = "//span[contains(text(),'AC')]")     
    private WebElement acFilter;
    @FindBy(xpath = "//span[contains(text(),'Sleeper')]") 
    private WebElement sleeperFilter;
    @FindBy(xpath = "//span[contains(text(),'10 AM - 5 PM')]") 
    private WebElement arrdeptime;
    ...
    public void applyOptionalFilter(WebDriver driver, String filterName) {
        ...
        switch (filterName.trim().toLowerCase()) {
            case "price":         
            	clickElement(driver, price);        
            	break;
            case "seats":        
            	clickElement(driver, seats);        
            	break;
            case "ratings":      
            	clickElement(driver, ratings);     
            	break;
            case "arrival time": 
            	clickElement(driver, arrivaltime);  
            	break;
            case "departure time":
            	clickElement(driver, depaturetime);
            	break;
            case "assured":       
            	clickElement(driver, assuredfilter);
            	break;
